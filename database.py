...
def create_database():
    ...
    # AI analyses for medical reports
    cursor.execute(
        """
        CREATE TABLE IF NOT EXISTS medical_report_analyses (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            patient_id INTEGER,
            report_id INTEGER,
            analysis_date TEXT,
    ...
    try:
        cursor.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_medicine_adherence_unique ON medicine_adherence(medicine_id, patient_id, date, scheduled_time)")
    except Exception:
        pass

    # Ensure unique email indexes to prevent duplicate patient and doctor accounts.
    try:
        cursor.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_patients_email ON patients(email)")
        cursor.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_doctors_email ON doctors(email)")
    except Exception:
        pass
...
def register_patient(name, email, password, age=None, blood_group=None, phone=None, emergency_contact=None, address=None, medical_history=None, allergies=None):
    ...
    cursor.execute("SELECT id FROM patients WHERE email=? LIMIT 1", (email.strip(),))
    if cursor.fetchone():
        conn.close()
    ...
    cursor.execute(
        ...
        (name.strip(), email.strip(), password, age, blood_group, phone, emergency_contact, address, medical_history, allergies)
    )
...
def register_doctor(name, email, password, specialization, phone=None):
    if not name or not isinstance(name, str) or not name.strip():
        raise ValueError("Doctor name is required")
    if not _validate_email(email):
        raise ValueError("Valid doctor email is required")
    if not password or not isinstance(password, str) or not password.strip():
        raise ValueError("Doctor password is required")
    if not specialization or not isinstance(specialization, str) or not specialization.strip():
        raise ValueError("Doctor specialization is required")
    if not _validate_phone(phone):
        raise ValueError("Valid doctor phone number is required")

    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()

    # Prevent duplicate email
    cursor.execute("SELECT id FROM doctors WHERE email=? LIMIT 1", (email.strip(),))
    if cursor.fetchone():
        conn.close()
        raise ValueError("Email already registered")
    ...
    cursor.execute(
        ...
        (name.strip(), email.strip(), password, phone, specialization.strip(), None, None)
    )
...
def insert_medical_report(patient_id, report_name, report_type, file_path, notes=None, upload_date=None):
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    if upload_date is None:
        upload_date = datetime.date.today().isoformat()
    cursor.execute(
        "INSERT INTO medical_reports (patient_id, report_name, report_type, file_path, upload_date, notes) VALUES (?, ?, ?, ?, ?, ?)",
        (patient_id, report_name, report_type, file_path, upload_date, notes)
    )
...
def get_reports_by_patient(patient_id):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, patient_id, report_name, report_type, file_path, upload_date, notes FROM medical_reports WHERE patient_id=? ORDER BY upload_date DESC",
        (patient_id,)
    )
    rows = cursor.fetchall()
    conn.close()
...
def delete_medical_report(report_id):
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    cursor.execute("SELECT file_path FROM medical_reports WHERE id=? LIMIT 1", (report_id,))
    row = cursor.fetchone()
    if row and row[0]:
        try:
            os.remove(row[0])
...
def insert_report_analysis(patient_id, report_id, analysis_date, summary, recommendations, details):
    """Insert an AI analysis for a medical report.

    `details` should be a JSON string containing the full analysis payload.
    """
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO medical_report_analyses (patient_id, report_id, analysis_date, summary, recommendations, details) VALUES (?, ?, ?, ?, ?, ?)",
        (patient_id, report_id, analysis_date, summary, recommendations, details)
    )
...
def get_analyses_by_report(report_id):
    """Return all analyses linked to a specific report ordered newest-first."""
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, patient_id, report_id, analysis_date, summary, recommendations, details FROM medical_report_analyses WHERE report_id=? ORDER BY analysis_date DESC",
        (report_id,)
    )
    rows = cursor.fetchall()
    conn.close()
...
def get_analyses_by_patient(patient_id):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, patient_id, report_id, analysis_date, summary, recommendations, details FROM medical_report_analyses WHERE patient_id=? ORDER BY analysis_date DESC",
        (patient_id,)
    )
    rows = cursor.fetchall()
    conn.close()
...
def delete_report_analysis(analysis_id):
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    cursor.execute("DELETE FROM medical_report_analyses WHERE id=?", (analysis_id,))
    conn.commit()
    conn.close()
...
def get_prescriptions_by_doctor(doctor_id, limit=None):
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    sql = (
        "SELECT p.id, p.patient_id, p.doctor_id, p.date, p.medicines, p.dosage, p.duration, p.notes, p.created_at, "
...
def get_analyses_by_patient_ids(patient_ids, limit=None):
    if not patient_ids:
        return []
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    ...
    sql = (
        "SELECT id, patient_id, report_id, analysis_date, summary, recommendations, details "
...
def create_medicine(patient_id, doctor_id, medicine_name, dosage, frequency, schedule_slots, food_instruction, start_date, end_date, notes=None, active=1):
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO medicines (patient_id, doctor_id, medicine_name, dosage, frequency, schedule_slots, food_instruction, start_date, end_date, notes, active, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ",
        (patient_id, doctor_id, medicine_name, dosage, frequency, json.dumps(schedule_slots or []), food_instruction, start_date, end_date, notes, active, datetime.datetime.now().isoformat())
    )
...
def get_medicine_by_id(medicine_id):
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    cursor.execute("SELECT id, patient_id, doctor_id, medicine_name, dosage, frequency, schedule_slots, food_instruction, start_date, end_date, notes, active, created_at, updated_at FROM medicines WHERE id=? LIMIT 1", (medicine_id,))
    row = cursor.fetchone()
    conn.close()
    return row


def get_medicines_by_patient(patient_id, active_only=True):
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    if active_only:
        cursor.execute(
            "SELECT id, patient_id, doctor_id, medicine_name, dosage, frequency, schedule_slots, food_instruction, start_date, end_date, notes, active, created_at, updated_at FROM medicines WHERE patient_id=? AND active=1 ORDER BY start_date DESC, medicine_name",
            (patient_id,)
        )
    else:
        cursor.execute(
            "SELECT id, patient_id, doctor_id, medicine_name, dosage, frequency, schedule_slots, food_instruction, start_date, end_date, notes, active, created_at, updated_at FROM medicines WHERE patient_id=? ORDER BY start_date DESC, medicine_name",
            (patient_id,)
        )
    rows = cursor.fetchall()
    conn.close()
...
def get_medicines_by_doctor(doctor_id):
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, patient_id, doctor_id, medicine_name, dosage, frequency, schedule_slots, food_instruction, start_date, end_date, notes, active, created_at, updated_at FROM medicines WHERE doctor_id=? ORDER BY start_date DESC, patient_id",
        (doctor_id,)
    )
    rows = cursor.fetchall()
    conn.close()
...
def update_medicine(medicine_id, medicine_name, dosage, frequency, schedule_slots, food_instruction, start_date, end_date, notes=None, active=1):
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE medicines SET medicine_name=?, dosage=?, frequency=?, schedule_slots=?, food_instruction=?, start_date=?, end_date=?, notes=?, active=?, updated_at=? WHERE id=?",
        (medicine_name, dosage, frequency, json.dumps(schedule_slots or []), food_instruction, start_date, end_date, notes, active, datetime.datetime.now().isoformat(), medicine_id)
    )
    conn.commit()
    conn.close()
...
def delete_medicine(medicine_id):
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    cursor.execute("DELETE FROM medicine_adherence WHERE medicine_id=?", (medicine_id,))
    cursor.execute("DELETE FROM medicines WHERE id=?", (medicine_id,))
    conn.commit()
    conn.close()


def record_medicine_adherence(medicine_id, patient_id, date, scheduled_time, status, note=None):
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    cursor.execute(
        "INSERT OR REPLACE INTO medicine_adherence (medicine_id, patient_id, date, scheduled_time, status, note, recorded_at) VALUES ((SELECT id FROM medicines WHERE id=?), ?, ?, ?, ?, ?, ?) ",
        (medicine_id, patient_id, date, scheduled_time, status, note, datetime.datetime.now().isoformat())
    )
    conn.commit()
    conn.close()


def get_adherence_history(patient_id=None, medicine_id=None, since_date=None, until_date=None, limit=None):
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    sql = "SELECT id, medicine_id, patient_id, date, scheduled_time, status, note, recorded_at FROM medicine_adherence"
    clauses = []
    params = []
    ...
    sql += " ORDER BY date DESC, scheduled_time DESC"
...
def get_patient_active_medicines(patient_id):
    return get_medicines_by_patient(patient_id, active_only=True)


def get_medicine_adherence_summary(patient_id):
    rows = get_medicines_by_patient(patient_id, active_only=False)
    adherences = get_adherence_history(patient_id=patient_id)
    return rows, adherences
...
def add_emergency_contact(patient_id, name, relationship, phone):
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    cursor.execute(
...
def get_emergency_contacts(patient_id):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, patient_id, name, relationship, phone, created_at FROM emergency_contacts WHERE patient_id=? ORDER BY created_at DESC",
        (patient_id,)
    )
...
def update_emergency_contact(contact_id, name=None, relationship=None, phone=None):
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    fields = []
    params = []
    ...
    sql = f"UPDATE emergency_contacts SET {', '.join(fields)} WHERE id=?"
    cursor.execute(sql, tuple(params))
    conn.commit()
    conn.close()
...
def create_sos_event(patient_id, reason, status='Sent', doctor_notified=0, doctor_id=None):
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    if doctor_id is None:
        cursor.execute(
            "INSERT INTO sos_events (patient_id, reason, status, doctor_notified) VALUES (?, ?, ?, ?)",
            (patient_id, reason, status, 1 if doctor_notified else 0)
        )
    else:
        cursor.execute(
            "INSERT INTO sos_events (patient_id, doctor_id, reason, status, doctor_notified) VALUES (?, ?, ?, ?, ?)",
            (patient_id, doctor_id, reason, status, 1 if doctor_notified else 0)
        )
    conn.commit()
    conn.close()


def get_sos_events(patient_id=None, limit=None):
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    sql = "SELECT id, patient_id, doctor_id, reason, created_at, status, doctor_notified FROM sos_events"
    params = []
    if patient_id is not None:
...
def get_recent_sos_alerts(limit=5):
    return get_sos_events(limit=limit)
